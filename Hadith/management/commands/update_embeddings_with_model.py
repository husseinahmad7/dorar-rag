# Hadith/management/commands/update_embeddings_with_model.py
...
class Command(BaseCommand):
    help = 'Update embeddings with a specific model'
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--model', type=str, default=None, help='Embedding model to use')
        parser.add_argument('--reset', action='store_true', help='Reset ChromaDB collection before updating')
    ...
    def handle(self, **options):
        ...
        if model:
            ...
            self.stdout.write(self.style.SUCCESS(f'Using embedding model: {model}'))
        ...
        # Reset ChromaDB collection if requested
        if reset:
            try:
    ...
    def process_batch(self, batch, vector_store):
        for hadith in batch:
            try:
                ...
                # Generate embedding
                embedding = vector_store.generate_embedding(text_without_tashkeel, use_without_tashkeel=False)
